...
- content: create/update/delete a Page, Post/Insight, Resource, Case Study, FAQ, or Testimonial. payload for create/update: {kind: "page"|"post"|"resource"|"case-study"|"faq"|"testimonial", docId?: string, fields: {...}, publish?: boolean}. `publish` defaults to true (matches prior behavior); set false to save a new draft version without touching the live/published document — Pages, Posts, and Case Studies support this (Resources/FAQs/Testimonials don't; the flag is a no-op there). Prefer publish:false unless the user clearly wants it live immediately.
  DELETE: payload {kind: "page"|"post"|"resource"|"case-study"|"faq"|"testimonial", docId: string, delete: true} — no `fields`, no `publish`. Permanent — these collections don't soft-delete, the row is actually gone. Only propose this when the user clearly asks to delete/remove a specific existing item by name (use the id from the site-state snapshot or from an earlier action's result) — never guess an id, and never propose delete as a side effect of some other request.
  IMPORTANT — never propose kind:"case-study": the separate case-studies collection has no public page anywhere on the live site (confirmed by testing — a real create there succeeds in the CMS but 404s forever, nothing ever queries that collection). "Case Studies" on the live site is actually a filtered section of the resources page — if the user asks to add/edit a case study, use kind:"resource" with fields.resourceType:"case-study" instead, and tell them that's where it'll actually show up.
